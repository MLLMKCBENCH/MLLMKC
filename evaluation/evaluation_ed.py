...
def compare_jsonl_files(file_a_path, file_b_path):
    # 初始化统计变量
    total_count = 0
    ...
    with open(file_a_path, 'r', encoding='utf-8') as a_file:
        for line in a_file:
            try:
                ...
                if a_prediction == a_data.get("mcq_dis_answer1"):
                    filtered_count += 1
                    continue
                ...
                b_disanswer1=b_item.get('mcq_dis_answer1')
                # 提取答案
...
def compare_jsonl_files1(file_a_path, file_b_path):
    # 初始化统计变量
    total_count = 0
    filtered_count = 0  # 被过滤掉的条目数
    match_count = 0
        ...
        for line in a_file:
            try:
                ...
                if a_prediction == a_data.get("mcq_dis_answer1"):
                    filtered_count += 1
                    continue
                if a_prediction == a_data.get("mcq_dis_answer2"):
                    filtered_count += 1
                    continue
                ...
                b_prediction = b_item.get('prediction')
                b_disanswer1=b_item.get('mcq_dis_answer1')
                b_disanswer2=b_item.get('mcq_dis_answer2')
                # 提取答案
                a_answer = extract_answer(a_prediction)
                b_answer = extract_answer(b_prediction)
...
if __name__ == "__main__":
    ...
    # 调用比较函数 
        results = compare_jsonl_files(args.file_a,  args.file_b) 
    if args.conflict_type=='ee':
        results = compare_jsonl_files1(args.file_a,  args.file_b) 
